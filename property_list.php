    ...
    if( $city_result && mysqli_num_rows($city_result)==1 ) //When $result1 is not false, and it contains single row.
    {
      ...
      if( isset($_GET["filter"]) )
      {
        if( $_GET["filter"]=="rent_desc" )
        {
          ...
          $sql_query2 = "SELECT id, name, address, gender, rent, rating_clean, rating_food, rating_safety, image_url FROM properties WHERE city_id='$city_id';";
...
//              $first_image = isset($images[0]) ? $images[0] : 'img/default.jpg'; // Use the first image or a default image
              ...
              $image_path = $property_row["image_url"]; // Fetch the image path from the database
